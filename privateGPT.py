...
def main():
    ...
    embeddings = HuggingFaceEmbeddings(
        model_name=embeddings_model_name, model_kwargs=embeddings_kwargs
    )
    db = Chroma(
    ...
    match model_type:
        case "LlamaCpp":
            llm = LlamaCpp(
                model_path="/data/privateGPT-gpu/models/llama-2-7b-chat.ggmlv3.q4_0.bin",
                n_ctx=model_n_ctx,
                callbacks=callbacks,
                verbose=False,
        ...
        case "GPT4All":
            if is_gpu_enabled:
                print(
                    "GPU is enabled, but GPT4All does not support GPU acceleration. Please use LlamaCpp instead."
                )
                exit(1)
            llm = GPT4All(
                model="/data/privateGPT-gpu/models/ggml-gpt4all-j-v1.3-groovy.bin",
                n_ctx=model_n_ctx,
                backend="gptj",
                callbacks=callbacks,
                verbose=False,
        ...
        case "MedLlama":
            llm = HuggingFacePipeline.from_model_id(
                model_id="/data/privateGPT-gpu/models/medllama",
        ...
        case "phi":
            llm = HuggingFacePipeline.from_model_id(
                model_id="/data/privateGPT-gpu/models/phi-1_5",
                task="text-generation",
                model_kwargs={
                    "trust_remote_code": True,
                    "torch_dtype": "auto",
        ...
        case "codegeex2":
            llm = HuggingFacePipeline.from_model_id(
                model_id="/data/privateGPT-gpu/models/codegeex2-6b",
                task="text-generation",
                device=1,
                model_kwargs={
                    "trust_remote_code": True,
        ...
        case "codellama":
            llm = HuggingFacePipeline.from_model_id(
                model_id="/data/privateGPT-gpu/models/CodeLlama-7b-hf",
                task="text-generation",
                device=1,
                model_kwargs={
                    "trust_remote_code": True,
        ...
        case "vicuna":
            llm = HuggingFacePipeline.from_model_id(
                model_id="/data/privateGPT-gpu/models/vicuna-7b-v1.5",
                task="text-generation",
                device=1,
                model_kwargs={
                    "trust_remote_code": True,
    ...
    while True:
        ...
        with open("/data/privateGPT-gpu/logs/vram_usage.log", "a") as f:
            f.write(
                f"{torch.cuda.memory_allocated()//1024**2} MB\t{psutil.virtual_memory().used//1024**2} MB\t{psutil.cpu_percent()}%\t{model_type}\t{model_n_ctx}\t{int(end - start)}\n"
            )
